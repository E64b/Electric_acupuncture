...
uint16_t getExit(const uint8_t *pChar, uint8_t index) { return pChar[index]; }

uint32_t getTime(uint8_t timeS) { return timeS * 1000; }
...
void work() {
  if (data.work && data.error == false) {
    ...
    if (data.program >= 3 && data.program <= 22 && data.step < ALL_DATA &&
        millis() - data.currentMillis >= getTime(DataMem.setTimeToStep)) {
      data.currentMillis = millis();
